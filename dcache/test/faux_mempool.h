...
public:
    CountingPool(bool all_local) : locality(all_local) {}
    ...
    void Deallocate(rdma_ptr<T> p, int size = 1){
        ...
        REMUS_ASSERT(allocat.at(add) == bytes, "Found free with ptr {} with wrong size (actual={} != expected={})", p, allocat.at(add), bytes);
        allocat.erase(add);
        free((T*) p.address());
