...
files = [[f"1t_{nodes}n_3k_one_sided-stats", 
         f"2t_{nodes}n_3k_one_sided-stats", 
         f"3t_{nodes}n_3k_one_sided-stats", 
         f"4t_{nodes}n_3k_one_sided-stats"],
         [f"1t_{nodes}n_3k_two_sided-stats", 
         f"2t_{nodes}n_3k_two_sided-stats", 
         f"3t_{nodes}n_3k_two_sided-stats", 
         f"4t_{nodes}n_3k_two_sided-stats"]]
title = "IHT Client Throughput Graph"
threads = [1, 2, 3, 4]
subtitle = f"(r320 [mlnx-4], {nodes} nodes, 60 seconds runtime)"
x_label = "Threads"
...
for group in files:
    ...
    for i, exp in enumerate(group_data):
        ...
        if int(exp['thread_count']) != threads[i] or exp[group_difference] != id:
            print(group)
            print(i)
            print(group[i])
        assert(int(exp['thread_count']) == threads[i])
