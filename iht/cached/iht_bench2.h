...
typedef TunedRdmaIHT<int, int, CNF_ELIST_SIZE, CNF_PLIST_SIZE> KVStoreTuned;

inline void iht2_run(BenchmarkParams& params, rdma_capability* capability, RemoteCache* cache, Peer& host, Peer& self){
    ...
    if (params.node_id == 0){
        ...
        threads.emplace_back(std::thread([&](){
            ...
            Peer p = Peer();
            KVStoreTuned iht = KVStoreTuned(p, CacheDepth::None, pool);
    ...
    for(int i = 0; i < params.thread_count; i++){
        threads.emplace_back(std::thread([&](int thread_index){
            ...
            std::shared_ptr<KVStoreTuned> iht = std::make_shared<KVStoreTuned>(self, params.cache_depth, pool);
            ...
            MapAPI* iht_as_map = new MapAPI(
                [&](MapCodes code, int param1, int param2, int param3){
                    if (code == Prepare){
                        ...
                        populate_amount = iht->populate(pool, param1, param2, param3, [=](int key){ return key; });
                        ExperimentManager::ClientArriveBarrier(endpoint);
                        ExperimentManager::ClientArriveBarrier(endpoint);
                        // cache->print_metrics();
                        // cache->reset_metrics();
    ...
    save_result("iht_tuned_result.csv", workload_results, params, params.thread_count);
}
